...
if __name__ == '__main__':
    with open('data.txt', 'r') as index_json:
        index = json.loads(index_json.read())
    ...
    for word in words:
        ...
        for url in index[word]:
            if url not in urls_hit:
                urls_hit[url] = 0
            urls_hit[url] = index[word][url]
    for link in sorted(urls_hit, key=lambda x: urls_hit[x]):
        print(link)
